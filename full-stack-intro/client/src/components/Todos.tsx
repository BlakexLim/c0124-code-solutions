...
export function Todos() {
  ...
  async function fetchData() {
    try {
      ...
      if (!res.ok) throw new Error(`${res.status} Network response was not ok`);
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      const post = await fetch('/api/todos', reqOpt);
      if (!post.ok) throw new Error(`Fetch error, status ${post.status}`);
      const postJson = await post.json();
      setTodos([...todos, postJson]);
    } catch (err) {
      setError(err);
    } finally {
      setIsLoading(false);
  ...
  async function toggleCompleted(todo: Todo) {
    try {
      ...
      const put = await fetch(`api/todos/${todo.todoId}`, reqOpt);
      if (!put.ok) throw new Error(`Fetch error, status ${put.status}`);
      const putJson = await put.json();
      const newTodos = todos.map((t) =>
        t.todoId === putJson.todoId ? putJson : t
      );
      setTodos(newTodos);
    } catch (err) {
      setError(err);
